...
const suggestionsSchema = z.object({
  suggestion: z.string().describe("The code to insert at cursor, or empty string of no completion needed")
});
...
export async function POST(requeste: NextRequest) {
  try {
    const { userId } = await auth();
    const { fileName, code, currentLine, textAfterCursor, textBeforeCursor, previousLines, nextLines, lineNumber } = await requeste.json();
