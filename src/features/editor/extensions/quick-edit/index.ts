...
export const quickEditState = StateField.define<boolean>({
  ...
  update(value, transaction) {
    for (const effect of transaction.effects) {
      if (effect.is(showQuickEditEffect)) return effect.value ?? false;
    }
  ...
  return [
    {
      ...
      create() {
        ...
        submitButton.className = "font-sans p-1 px-2 text-muted-foreground hove:text-foreground hover:bg-foreground/10 rounded-sm";
