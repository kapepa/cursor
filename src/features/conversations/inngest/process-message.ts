...
export const processMessage = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      if (internalKey) {
        await step.run("updateMessageContent", async () => {
          await convex.mutation(api.system.updateMessageContent, {
            internalKey,
            messageId,
            content: "My aplogies, I encountered an error while processing your request. Let me know if you need anything else!"
          })
        })
      }
    }
