  ...
  return [
    {
      ...
      create() {
        ...
        dom.className = "bg-popover text-popover-foreground z-50 rounded-sm border border-input p-1 shadow-md items-center gap-2 text-sm";

        const addToChatButton = document.createElement("button");
        addToChatButton.textContent = "Add to Chat";
        addToChatButton.classList = "font-sens p-1 px-2 hover:bg-foreground/10 rounded-sm";

        const quickEditButton = document.createElement("button");
        quickEditButton.classList = "font-sens p-1 px-2 hover:bg-foreground/10 rounded-sm flex items-center gap-1";
...
const selectionTooltipField = StateField.define<readonly Tooltip[]>({
  ...
  update(tooltips, transaction) {
    if (transaction.docChanged || transaction.selection) return createTooltipForSelection(transaction.state)
    for (const effect of transaction.effects) {
      if (effect.is(showQuickEditEffect)) return createTooltipForSelection(transaction.state);
    }
    return tooltips;
  },
